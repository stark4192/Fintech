...
public class InvoiceDaoImpl {
	...
	public int insertInvoice(int customerOrderId, int salesOrg, String distributionChannel, int customerNumber,
			int companyCode, String orderCurrency, Double amountInUSD, String orderCreationDate) {
		int status = 0;
		try (Connection connection = DatabaseConnection.connect()) {
			...
			PreparedStatement ps = connection.prepareStatement(
					"insert into h2h_oap(customer_Order_Id,sales_Org, distribution_Channel,company_Code,order_Creation_Date, order_Currency,customer_Number,amount_In_USD) values(?,?,?,?,?,?,?,?) ");
	...
	public int updateInvoice(int id, String distributionChannel, int companyCode, String orderCurrency) {
		...
		try (Connection connection = DatabaseConnection.connect()) {
			...
			// Execute a query
			PreparedStatement ps = connection.prepareStatement(
					"update h2h_oap  distribution_Channel=?,company_Code=?,order_Currency=? where Sl_no=?");
